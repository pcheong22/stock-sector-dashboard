...
The HTML file has all data embedded as JSON -- no server needed.
The journal tab calls the GitHub API directly from the browser using
a token the user saves in their own browser's localStorage.
"""
...
def build_html() -> str:
    ...
    repo        = "pcheong22/stock-sector-dashboard"   # update if repo name differs
...
<div id="journal" class="panel">
  ...
  <p class="field" style="margin-bottom:4px">GitHub token — saved in your browser only, used only to write to your repo:</p>
  <div class="token-row">
  ...
  <div class="form-grid">
    ...
    <div class="form-group">
      <label class="field">Did the sector rankings change this vs what you'd have done otherwise?</label>
  ...
  <details>
    ...
    <button class="btn muted" style="margin-bottom:10px;margin-top:8px" onclick="loadJournal()">Load entries</button>
...
const DB      = {payload};
const REPO    = "{repo}";
const ETF_MAP = {etf_map};
const JOURNAL = "data/decision_journal.csv";
...
function updateTokenStatus() {{
  ...
  el.textContent = getToken() ? "✓ Token saved in this browser." : "No token saved yet.";
  if (getToken()) document.getElementById("gh-token").placeholder = "token saved — paste new one to replace";
}}
...
async function ghPut(path, content, sha, message) {{
  ...
  if (sha) body.sha = sha;
  const r = await fetch(`https://api.github.com/repos/${{REPO}}/contents/${{path}}`,
    {{method:"PUT", headers:{{"Authorization":`Bearer ${{getToken()}}`,"Accept":"application/vnd.github+json","Content-Type":"application/json"}},
...
async function loadJournal() {{
  ...
  try {{
    const existing = await ghGet(JOURNAL);
    if (!existing) {{ document.getElementById("journal-table").innerHTML = "<tr><td>No entries yet.</td></tr>"; return; }}
    const text = decodeURIComponent(escape(atob(existing.content.replace(/\\n/g,""))));
    const lines = text.trim().split("\\n");
    const cols  = lines[0].split(",");
    const rows  = lines.slice(1).reverse().map(l => l.split(","));
    buildTable(document.getElementById("journal-table"), cols, rows, 0);
  }} catch(e) {{
    alert(e.message);
  }}
}}
...
// ── table builder ─────────────────────────────────────────────────────────
function buildTable(tableEl, cols, rows, rankOffset=1) {{
  let sortCol=-1, sortAsc=false;
  function render(data) {{
    const head = "<thead><tr>" + (rankOffset?"<th>#</th>":"") +
      cols.map((c,i) => `<th data-i="${{i}}">${{c}}</th>`).join("") + "</tr></thead>";
    const body = "<tbody>" + data.map((row,ri) =>
...
function buildBars(el, rows, cols) {{
  ...
  el.innerHTML = rows.map(r => {{
    ...
    return `<div class="bar-row"><div class="bar-label">${{r[si]}}</div>
      <div class="bar-track"><div class="bar-fill" style="width:${{pct}}%">${{Number(r[vi]).toFixed(1)}}</div></div></div>`;
  }}).join("");
}}
...
function render(dateStr) {{
  ...
  document.getElementById("sector-caption").textContent =
    hasPrior ? "Changes vs. prior stored date." : "No prior date for comparison yet.";
